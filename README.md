...
FUN-кредиты). Действия — кнопки самой игры: «спин» (Space, та же ставка) и кнопки ставок
`FUN 0.10 … СТАВКА` (в играх Amusnet клик по ставке = поставить и крутить). Награда за спин —
изменение баланса, делённое на ставку, обрезанное в [−1, 3]: проигрыш ставки → −1 → ток в PPL101
(аверсивный дофамин), выигрыш → + → ток в PAM-кластер (316 нейронов, дофамин награды).
Каждый спин пишется в `data/dopamine.csv` (ставка, баланс до/после, дельта, дофамин ±, накопленная
кривая); в визуализации — плитки баланса и кривая дофамина.
...
(`on=0` — выключить; уже подключённые удалённые зрители отваливаются). Адрес для коллег печатается
при старте: `http://<ваш-IP>:8765/`. При первом запуске Windows спросит разрешение брандмауэра для
python — разрешить для частных сетей (или `netsh advfirewall firewall add rule name=FlyBrain
dir=in action=allow protocol=TCP localport=8765`). Доступ только на просмотр: управления в странице нет.
